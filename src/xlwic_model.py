...
class XLWiCModel(nn.Module):
    def __init__(self, conf) -> None:
        super().__init__()
        self.encoder = AutoModel.from_pretrained(conf.model.pretrained_model_name)
        ...
        if conf.model.words_combiner == 'sum':
    ...
    def forward(self, input_ids, indices_mask, labels=None):
        ...
        hidden_states = hidden_states.hidden_states[:, hs_indices]
        ...
        if self.conf.model.subword_combiner == 'sum':
        ...
        if self.conf.model.words_combiner == 'sum':
